...
def apply_entropy_and_morphology(img):
    ...
    Returns:
    Binary image after entropy-based thresholding and morphological operations
    """
    ...
    try:
        ...
        # Fallback to Otsu if entropy fails
...
class VTKWidget(QWidget):
    ...
    def create_3d_from_binary(self, binary_data, skull_data, brain_tissue_data, csf_data):
        """Create a 3D representation with hemorrhage, skull, brain tissue, and CSF."""
        self.renderer.RemoveAllViewProps()
        height, width = binary_data.shape
        depth = 20
        dims = (width, height, depth)

        hemorrhage_volume = self.create_volume_data(binary_data, dims, vtk.VTK_UNSIGNED_CHAR)
        self.add_volume_rendering(hemorrhage_volume, color=(1.0, 0.0, 0.0),
                                  opacity_points=[(0, 0.0), (128, 0.3), (255, 0.8)])

        self.create_3d_from_skull(skull_data, dims)
        self.create_3d_from_brain_tissue(brain_tissue_data, dims)

        csf_volume = self.create_volume_data(csf_data, dims, vtk.VTK_UNSIGNED_CHAR)
        self.add_volume_rendering(csf_volume, color=(0.0, 0.0, 1.0), opacity_points=[(0, 0.0), (128, 0.2), (255, 0.5)])

        outline = vtk.vtkOutlineFilter()
        outline.SetInputData(hemorrhage_volume)
    ...
    def create_volume_data(self, data, dims, scalar_type):
        ...
        volume_data = np.zeros((width, height, depth),
                               dtype=np.uint8 if scalar_type == vtk.VTK_UNSIGNED_CHAR else np.int16)
    ...
    def create_3d_from_skull(self, skull_data, dims):
        """Create a 3D surface model of the skull using Marching Cubes."""
        skull_volume = self.create_volume_data(skull_data, dims, vtk.VTK_UNSIGNED_CHAR)
    ...
    def create_3d_from_brain_tissue(self, brain_tissue_data, dims):
        """Create a 3D volume model of brain tissue with opacity-based gradient."""
        brain_volume = self.create_volume_data(brain_tissue_data, dims, vtk.VTK_UNSIGNED_CHAR)
...
class BrainCTViewer(QMainWindow):
    def __init__(self):
        ...
        self.current_folder = ""
        self.thumbnails = []  # Store thumbnail labels and slice indices
        self.initUI()
    ...
    def generate_thumbnails(self):
        """Generate thumbnail previews for axial slices."""
        ...
        slice_step = max(1, num_slices // 20)  # Show ~20 thumbnails max
    ...
    def update_thumbnail_highlight(self):
        """Highlight the thumbnail corresponding to the current slice."""
    ...
    def on_thumbnail_clicked(self, slice_index):
        """Handle thumbnail click to jump to the corresponding slice."""
    ...
    def update_window(self):
        ...
        if self.current_volume is not None:
            self.update_slice(self.current_slice_index)
            self.generate_thumbnails()  # Regenerate thumbnails with new windowing

    def process_slice(self, img):
        """Process a slice with all preprocessing steps, including hemorrhage, skull, brain tissue, and CSF."""
        ...
        self.vtk_widget.create_3d_from_binary(hemorrhage_mask, skull_mask, brain_tissue_mask, csf_mask)
    ...
    def export_images(self):
        ...
        original_path = f"{save_dir}/original_slice_{self.current_slice_index + 1}.png"
        ...
        if self.current_axial_slice is not None:
            axial_path = f"{save_dir}/axial_slice_{self.current_slice_index + 1}.png"
        ...
        if self.current_coronal_slice is not None:
            ...
            coronal_path = f"{save_dir}/coronal_slice.png"
        ...
        if self.current_sagittal_slice is not None:
            ...
            sagittal_path = f"{save_dir}/sagittal_slice.png"
    ...
    def show_axial_coronal_sagittal(self):
        """Display axial, coronal, and sagittal views of the current volume."""
        if self.current_volume is None:
